...
const IpSecSetting = () => {
  ...
  const handleApiResponse = async () => {
    ...
    try {
      ...
      const profileData = {
        ...
        SubnetLocalGateway: formData.SubnetLocalGatewayIPv4 || formData.SubnetLocalGatewayIPv6,
        ...
        RemoteGateway: connectionCount === 1
            ...
            : Array.from({ length: connectionCount }, (_, index) =>
                formData[`RemoteGatewayIPv4_${index}`] || formData[`RemoteGatewayIPv6_${index}`]
            ).filter(Boolean).join(';'),  // Lọc null/undefined và join

        // Tương tự cho SubnetRemoteGateway
        SubnetRemoteGateway: connectionCount === 1
            ? (formData.SubnetRemoteGatewayIPv4 || formData.SubnetRemoteGatewayIPv6)
            : Array.from({ length: connectionCount }, (_, index) =>
                formData[`SubnetRemoteGatewayIPv4_${index}`] || formData[`SubnetRemoteGatewayIPv6_${index}`]
            ).filter(Boolean).join(';'),

        IKEVersion: formData.IKE2Version,
        Mode: formData.IKEMode,
        ESPAHProtocol: formData.ESPAHProtocol,
        IKEReauthTime: formData.IKEReauthTime ? parseInt(formData.IKEReauthTime, 10) : null,
  ...
  const next = async () => {
    if (current === 0 && !certificationValid) {
      return;
    }
