...
const { getAllDeviceLogs, getDeviceLogByLogType, deleteDeviceLogsByIds, getDeviceLogsByIds, getDeviceLogsByTypeAndTimeRange, deleteDeviceLogsByTypeAndTimeRange } = require("../models/DeviceLogs.model");
...
exports.exportLogTrafficbyLogTypeAndLogName = async (req, res) => {
  try {
    const logType = req.params.logType;
    const logFiles = req.params.logNames.split(",");
    ...
    const createAndStyleWorksheet = (workbook, file, logType) => {
      ...
      rows.forEach((row, rowIndex) => {
        ...
        // Style data rows: alternating colors, full borders, left-aligned
        const rowColor = rowIndex % 2 === 0 ? "FFF9F9F9" : "FFFFFFFF"; // Light gray for even rows
        wsRow.eachCell((cell) => {
      ...
      // Style headers: bold, background color, centered - specific colors
      const headerRow = worksheet.getRow(1);
      headerRow.eachCell((cell, colNumber) => {
        const header = headers[colNumber - 1].header;
        let headerColor = "FF4F81BD"; // Default blue
        if (header === "SRC_IP" || header === "DST_IP") {
          headerColor = "FF4472C4"; // Darker blue for IP columns
        } else if (header === "SRC_PORT" || header === "DST_PORT") {
          headerColor = "FF70AD47"; // Green for Port columns
        }
    ...
    if (formatType === "zip-excel") {
      ...
      for (const file of logFiles) {
        ...
        createAndStyleWorksheet(workbook, file, logType);
    ...
    if (logFiles.length === 1 && (formatType === "txt" || formatType === "xlsx")) {
      ...
      if (formatType === "xlsx") {
        ...
        res.setHeader("Content-Disposition", `attachment; filename=Sysnetdef_Logs_${logType}_${file}.xlsx`);
...
exports.deleteLogTrafficbyLogTypeAndLogName = async (req, res) => {
  try {
    ...
    for (const file of logFiles) {
      const filePath = path.join(logPath, file.trim());
      if (fs.existsSync(filePath)) {
...
exports.exportDeviceLogByLogTypeAndIds = async (req, res) => {
  try {
    ...
    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", `attachment; filename=activity_logs_${logType}_${new Date().toISOString().slice(0, 10)}.xlsx`);
    await workbook.xlsx.write(res);
  } catch (error) {
    console.log("error ", error);
...
exports.exportDeviceLogByTypeAndTimeRange = async (req, res) => {
  try {
    ...
    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", `attachment; filename=device_logs_${logType}_${from}_to_${to}.xlsx`);
    await workbook.xlsx.write(res);
    res.end();
  } catch (error) {
