...
import {
  ...
  importDeleteBulkProtectedAddresses,
...
export default function NetworkProtection() {
  ...
  useEffect(() => {
    async function fetchData() {
      const res = await getAllProtectedAddresses();
      setIpList(res.data || []);
    }
    fetchData();
  }, [refreshKey]);
  ...
  const handleCreate = async (newIp) => {
    try {
      ...
      await insertProtectedAddresses({
  ...
  const handleDeleteConfirm = async () => {
    setIsImportLoading(true); // Bật trạng thái loading
    const totalProcessingTime = selectedRowKeys.length * 2;
    setDeleteEstimatedTime(totalProcessingTime);
    setDeleteCountdown(totalProcessingTime);
  ...
  const handleImportDeleteApply = async () => {
    ...
    try {
      ...
      await importDeleteBulkProtectedAddresses(toDeleteObjects);
      const res = await getAllProtectedAddresses();
      if (res && res.data) setIpList(res.data);
      setSelectedRowKeys([]);
      message.success(
  ...
  return (
    <InterfaceLayout selectedKey="protected">
      <Title level={3}>Network Protecting</Title>
      <Paragraph>
        Enhance your network security by adding specific IP address of Server to
        the SysnetDef device. The addresses inside the list are being protected.
      </Paragraph>
      <Space>
        <Button
          icon={<PlusOutlined />}
