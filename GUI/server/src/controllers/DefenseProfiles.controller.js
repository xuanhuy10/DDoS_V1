...
const { getAllDeviceInterfaces, updateDeviceInterface } = require("../models/DeviceInterfaces.model");
const {
...
const buildConfigPackage = (config, currentConfig, portName = "PORT1") => {
  ...
  progressLog(COLORS.spacing, `Total time to process for ${portName}: `, COLORS.blue, `${totalProcessingTimeInSeconds} seconds`, COLORS.reset);
...
const checkDuplicateName = async (name) => {
  ...
  return profiles.some((profile) => profile.DefenseProfileName.toLowerCase() === name.toLowerCase());
};
/**
...
exports.getDefenseProfiles = async (req, res) => {
  config.progressLog(config.COLORS.cyan, "\n[PROGRESS] : Get Defense Profiles ...", config.COLORS.reset);
  try {
...
exports.getAllDefenseProfilesByActive = async (req, res) => {
  progressLog(config.COLORS.cyan, "\n[PROGRESS] : Get Defense Profiles By Active ...", config.COLORS.reset);
  try {
    const defenseProfiles = await getActiveDefenseProfile();
    return res.status(200).json({ data: defenseProfiles });
  } catch (error) {
    console.log("error ", error);
    return res.status(500).json({ message: "Get all active defense profiles failed" });
  }
};
/**
...
exports.getDefenseProfileAttackTypesConfig = async (req, res) => {
  progressLog(config.COLORS.cyan, "\n[PROGRESS] : Get Defense Profile Attack Type " + req.params.attackType + " Config ...", config.COLORS.reset);
  try {
...
exports.getAllThresholdByActiveDefenseProfile = async (req, res) => {
  progressLog(config.COLORS.cyan, "\n[PROGRESS] : Get All Threshold By Active Defense Profile ...", config.COLORS.reset);
  try {
    const profileAllThreshold = await getAllThresholdByActiveDefenseProfile();
    if (!profileAllThreshold) {
      return res.status(404).json({ message: "No active defense profile found" });
    }
    profileAllThreshold.SYNFloodRate = profileAllThreshold.SYNFloodSYNThreshold / profileAllThreshold.DetectionTime;
    profileAllThreshold.DNSFloodRate = profileAllThreshold.DNSFloodThreshold / profileAllThreshold.DetectionTime;
    profileAllThreshold.IPSecFloodRate = profileAllThreshold.IPSecIKEThreshold / profileAllThreshold.DetectionTime;
...
exports.insertDefenseProfile = async (req, res) => {
  config.progressLog(config.COLORS.cyan, "\n[PROGRESS] : insert Defense Profile ...", config.COLORS.reset);
  let progressReport = "";
  try {
    ...
    if (await checkDuplicateName(defenseProfileData.DefenseProfileName)) {
      await insertSystemLogToDatabase(userId, "DefenseProfile", "Config", "Create Defense Profiles", `Create defense profile: ${defenseProfileData.DefenseProfileName}`, "Failed", "Profile name already exists");
    ...
    for (let key in defenseProfileData) {
      if (defenseProfileData[key] == null || defenseProfileData[key] === undefined) {
        return res.status(400).json({ message: `Missing field: ${key}` });
      }
    }
    defenseProfileData.UserId = userId;
    defenseProfileData.DefenseProfileCreateTime = format(currentDate, "yyyy/MM/dd HH:mm:ss", { locale: vi });
    defenseProfileData.DefenseProfileStatus = "Inactive";
    defenseProfileData.DefenseProfileType = "UserProfile";
    defenseProfileData.DefenseProfileLastModified = format(currentDate, "yyyy/MM/dd HH:mm:ss", { locale: vi });
    defenseProfileData.DefenseProfileUsingTime = null;
    const { configPackage, fieldsCount } = buildFullConfigPackage(defenseProfileData, "PORT1");
    if (!configPackage) {
      return res.status(400).json({
    ...
    if (!isMainCConnected) {
      await insertSystemLogToDatabase(userId, "DefenseProfile", "Config", "Create Defense Profiles", `Create defense profile: ${defenseProfileData.DefenseProfileName}`, "Failed", "Cannot connect to C program");
      return res.status(500).json({ message: "Cannot connect to C program" });
    }
    const timeout = Math.max(fieldsCount * 2 * 1000, 5000);
    config.progressLog(config.COLORS.spacing, `Set timeout for command: `, config.COLORS.blue, `${timeout / 1000} seconds`, config.COLORS.reset);
    ...
    if (isError) {
      await insertSystemLogToDatabase(userId, "DefenseProfile", "Config", "Create Defense Profiles", `Create defense profile: ${defenseProfileData.DefenseProfileName}`, "Failed", `C error: ${result}`);
      return res.status(500).json({
    ...
    // Thêm: Truy vấn lại profile sau insert để lấy bản ghi đầy đủ
    const updatedProfile = await getDefenseProfileByDefenseProfileId(profileId);
    await insertSystemLogToDatabase(userId, "DefenseProfile", "Config", "Create Defense Profiles", `Create defense profile: ${defenseProfileData.DefenseProfileName}`, "Success", null);
    return res.status(200).json({
      message: "Insert defense profile success",
      data: updatedProfile, // Thêm object profile đã tạo (thay vì chỉ profileId)
      progress: progressReport,
    });
  } catch (error) {
    console.log("error ", error);
    await insertSystemLogToDatabase(req.user.payload.Id, "DefenseProfile", "Config", "Create Defense Profiles", `Create defense profile`, "Failed", error.message);
    return res.status(500).json({
...
exports.updateDefenseProfile = async (req, res) => {
  config.progressLog(config.COLORS.cyan, "\n[PROGRESS] : Update Defense Profile ...", config.COLORS.reset);
  ...
  try {
    ...
    if (!currentProfile) {
      await insertSystemLogToDatabase(userId, "DefenseProfile", "Config", "Update Defense Profile", `Failed to update profile: ${profileId}`, "Failed", "Defense profile not found");
      return res.status(404).json({
        success: false,
    ...
    if (updatedData.DefenseProfileName && updatedData.DefenseProfileName !== currentProfile.DefenseProfileName) {
      const profiles = await getAllDefenseProfiles();
      if (profiles.some((profile) => profile.DefenseProfileName.toLowerCase() === updatedData.DefenseProfileName.toLowerCase() && profile.DefenseProfileId !== profileId)) {
        await insertSystemLogToDatabase(userId, "DefenseProfile", "Config", "Update Defense Profile", `Failed to update profile: ${updatedData.DefenseProfileName}`, "Failed", "Profile name already exists");
        return res.status(400).json({
          success: false,
          message: "Profile name already exists",
          progress: progressReport,
    ...
    if (usingInterfaces.length === 0) {
      progressLog(COLORS.spacing, `No interfaces found for profile ${profileId}, updating database only`, COLORS.reset);
      await updateDefenseProfile(profileId, mergedData);
      progressReport += `database updated`;
      // Thêm: Truy vấn lại profile sau cập nhật
      const updatedProfile = await getDefenseProfileByDefenseProfileId(profileId);
      await insertSystemLogToDatabase(userId, "DefenseProfile", "Config", "Update Defense Profile", `Updated profile: ${mergedData.DefenseProfileName}`, "Success", "No interfaces to update");
      return res.status(200).json({
        success: true,
        message: "Defense profile updated successfully (no interfaces to update)",
        progress: progressReport,
        estimatedTime: 0,
        data: updatedProfile, // Thêm object profile đã cập nhật
      });
    }
    const isMainCConnected = await checkMainCConnection();
    if (!isMainCConnected) {
      await insertSystemLogToDatabase(userId, "DefenseProfile", "Config", "Update Defense Profile", `Failed to update profile: ${profileId}`, "Failed", "Cannot connect to C program");
      return res.status(500).json({
        success: false,
    ...
    for (const iface of usingInterfaces) {
      const { config, differentFieldsCount } = buildConfigPackageFromDevice(iface, mergedData, false, currentProfile);
      if (config) {
        configPackages.push({ config, differentFieldsCount });
        totalProcessingTimeInSeconds += differentFieldsCount * 2;
      } else {
        progressLog(COLORS.spacing, `No config changes for interface ${iface.InterfaceName}`, COLORS.reset);
      }
    }
    progressLog(COLORS.spacing, `Total time to process for all ports: `, COLORS.blue, `${totalProcessingTimeInSeconds} seconds`, COLORS.reset);
    if (configPackages.length === 0) {
      progressLog(COLORS.spacing, `No changes detected, updating database only`, COLORS.reset);
      await updateDefenseProfile(profileId, mergedData);
      progressReport += `database updated`;
      // Thêm: Truy vấn lại profile sau cập nhật
      const updatedProfileNoChanges = await getDefenseProfileByDefenseProfileId(profileId);
      await insertSystemLogToDatabase(userId, "DefenseProfile", "Config", "Update Defense Profile", `Updated profile: ${mergedData.DefenseProfileName}`, "Success", "No changes to send to mainC");
      return res.status(200).json({
        success: true,
        message: "Defense profile updated successfully (no changes to mainC)",
        progress: progressReport,
        estimatedTime: 0,
        data: updatedProfileNoChanges, // Thêm object profile đã cập nhật
      });
    }
    for (const { config, differentFieldsCount } of configPackages) {
      ...
      const isError = config.isProduction ? result.includes("ERROR") : result.includes("ERROR");
      if (isError) {
        await insertSystemLogToDatabase(userId, "DefenseProfile", "Config", "Update Defense Profile", `Failed to update profile: ${profileId}`, "Failed", `C error: ${result}`);
        return res.status(500).json({
          success: false,
    ...
    // Thêm: Truy vấn lại profile sau cập nhật
    const updatedProfile = await getDefenseProfileByDefenseProfileId(profileId);
    await insertSystemLogToDatabase(userId, "DefenseProfile", "Config", "Update Defense Profile", `Updated profile: ${mergedData.DefenseProfileName}`, "Success", null);
    return res.status(200).json({
      success: true,
      message: "Defense profile updated successfully",
      progress: progressReport,
      estimatedTime: totalProcessingTimeInSeconds,
      data: updatedProfile, // Thêm object profile đã cập nhật (thay vì { profileId, changes })
    });
  } catch (error) {
    console.error("Update defense profile error:", error);
    await insertSystemLogToDatabase(userId, "DefenseProfile", "Config", "Update Defense Profile", `Failed to update profile: ${profileId}`, "Failed", error.message);
    return res.status(500).json({
      success: false,
...
exports.applyDefenseProfileToInterfaces = async (req, res) => {
  config.progressLog(config.COLORS.cyan, "\n[PROGRESS] : apply Defense Profile To Interfaces ...", config.COLORS.reset);
  let progressReport = "";
  try {
    ...
    for (const ifaceData of interfaces) {
      ...
      if (iface.DefenseProfileId && iface.DefenseProfileId !== profileId && name.toLowerCase().startsWith("eth")) {
        oldAssignments.push({
          interfaceName: name,
          oldProfileId: iface.DefenseProfileId,
        });
      }
      ...
      const currentConfig = iface.DefenseProfileId ? await getDefenseProfileByDefenseProfileId(iface.DefenseProfileId) : {};
      const result = buildConfigPackage(newProfile, currentConfig || {}, portName);
    ...
    config.progressLog(config.COLORS.spacing, `Total time to process for all ports: `, config.COLORS.blue, `${totalProcessingTimeInSeconds} seconds`, config.COLORS.reset);
    const isMainCConnected = await checkMainCConnection();
    if (!isMainCConnected) {
      await insertSystemLogToDatabase(req.user.payload.Id, "DefenseProfile", "Config", "Apply Defense Profiles to Interfaces", `Apply profile ${profileId}`, "Failed", "Cannot connect to C program");
      return res.status(500).json({
        message: "Cannot connect to C program",
        progress: progressReport,
      });
    ...
    for (const { configPackage, differentFieldsCount } of configPackages) {
      config.progressLog(config.COLORS.spacing, "Command sent to C program: ", config.COLORS.magenta, configPackage, config.COLORS.reset);
      const timeout = Math.max(differentFieldsCount * 2 * 1000, 5000);
      config.progressLog(config.COLORS.spacing, `Set timeout for command: `, config.COLORS.blue, `${timeout / 1000} seconds`, config.COLORS.reset);
      const result = await sendCommandToCProgram(configPackage, timeout);
      const isError = config.isProduction ? result.includes("ERROR") : result.includes("ERROR");
      if (isError) {
        await insertSystemLogToDatabase(req.user.payload.Id, "DefenseProfile", "Config", "Apply Defense Profiles to Interfaces", `Apply profile ${profileId}`, "Failed", `C error: ${result}`);
        progressReport = "";
        return res.status(500).json({
    ...
    for (const ifaceData of interfaces) {
      const iface = allInterfaces.find((i) => i.InterfaceName === ifaceData.name);
      await updateDeviceInterface(iface.InterfaceId, {
        ...iface,
        DefenseProfileId: profileId,
      });
    }
    for (const { interfaceName, oldProfileId } of oldAssignments) {
      const oldProfile = await getDefenseProfileByDefenseProfileId(oldProfileId);
      if (oldProfile) {
        let oldUsingTime = oldProfile.DefenseProfileUsingTime ? JSON.parse(oldProfile.DefenseProfileUsingTime) : [];
        oldUsingTime = oldUsingTime.filter((entry) => entry.name !== interfaceName);
        oldProfile.DefenseProfileUsingTime = oldUsingTime.length > 0 ? JSON.stringify(oldUsingTime) : null;
        oldProfile.DefenseProfileLastModified = format(new Date(), "yyyy/MM/dd HH:mm:ss", { locale: vi });
        await updateDefenseProfile(oldProfileId, oldProfile);
      }
    }
    ...
    newProfile.DefenseProfileLastModified = format(new Date(), "yyyy/MM/dd HH:mm:ss", { locale: vi });
    newProfile.DefenseProfileStatus = "Active";
    await updateDefenseProfile(profileId, newProfile);
    progressReport += `database updated`;
    const updatedProfile = await getDefenseProfileByDefenseProfileId(profileId);
    const updatedInterfaces = [];
    const freshAllInterfaces = await getAllDeviceInterfaces();
    for (const ifaceData of interfaces) {
      const updatedIface = freshAllInterfaces.find((i) => i.InterfaceName === ifaceData.name);
      if (updatedIface) {
        updatedInterfaces.push(updatedIface);
      }
    }
    await insertSystemLogToDatabase(req.user.payload.Id, "DefenseProfile", "Config", "Apply Defense Profiles to Interfaces", `Applied profile ${profileId} to interfaces: ${interfaces.map((i) => i.name).join(", ")}`, "Success", null);
    return res.status(200).json({
    ...
    console.log("error:", error);
    await insertSystemLogToDatabase(req.user.payload.Id, "DefenseProfile", "Config", "Apply Defense Profiles to Interfaces", `Apply profile ${req.params.profileId}`, "Failed", error.message);
    return res.status(500).json({
      message: "Failed to apply defense profile to interfaces",
      error: error.message,
      progress: progressReport,
...
exports.deleteDefenseProfile = async (req, res) => {
  config.progressLog(config.COLORS.cyan, "\n[PROGRESS] : delete Defense Profile ...", config.COLORS.reset);
  let progressReport = "";
  try {
    const profileId = req.params.profileId;
    const defenseProfileData = await getDefenseProfileByDefenseProfileId(profileId);
    if (!defenseProfileData) {
      await insertSystemLogToDatabase(req.user.payload.Id, "DefenseProfile", "Config", "Delete Defense Profiles", `Delete defense profile: ${defenseProfileData.DefenseProfileName}`, "Failed", "Defense profile not found");
      return res.status(404).json({
        message: "Defense profile not found",
        progress: progressReport,
      });
    ...
    if (defenseProfileData.DefenseProfileStatus === "Active") {
      await insertSystemLogToDatabase(req.user.payload.Id, "DefenseProfile", "Config", "Delete Defense Profiles", `Delete defense profile: ${defenseProfileData.DefenseProfileName}`, "Failed", "Defense profile is currently active. Please deactivate it before deleting.");
      return res.status(400).json({
        message: "Defense profile is currently active. Please deactivate it before deleting.",
        progress: progressReport,
      });
    }
    await deleteDefenseProfile(profileId);
    progressReport += "database updated";
    // Không query lại vì đã delete, nhưng giữ data là profile cũ để xác nhận
    const deletedProfile = { ...defenseProfileData, status: "Deleted" }; // Hoặc chỉ dùng defenseProfileData
    await insertSystemLogToDatabase(req.user.payload.Id, "DefenseProfile", "Config", "Delete Defense Profiles", `Delete defense profile: ${defenseProfileData.DefenseProfileName}`, "Success", null);
    return res.status(200).json({
      message: "Delete defense profile success",
      data: deletedProfile, // Thêm object profile đã xóa (info cũ)
      progress: progressReport,
    });
  } catch (error) {
    console.log("error ", error);
    await insertSystemLogToDatabase(req.user.payload.Id, "DefenseProfile", "Config", "Delete Defense Profiles", `Delete defense profile`, "Failed", error.message);
    return res.status(500).json({
