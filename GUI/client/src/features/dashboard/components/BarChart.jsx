...
import { useSummaryStore } from "@/store/trafficStore"; // Import store
import { useEffect } from "react";

const BarChart = ({ sdata: propSdata }) => {
  console.log("BarChart", propSdata);

  // Lấy state và actions từ store
  const { summaryData, updateSummaryData } = useSummaryStore();

  useEffect(() => {
    if (propSdata) {
      updateSummaryData(propSdata);
    }
  }, [propSdata, updateSummaryData]);

  const getOption = () => {
    const sdata = summaryData || {}; // Fallback nếu store empty
    ...
    const receivedDisplayData = receivedData.map((val, idx) =>
      ensureMinimumValue(val, val)
    );
    const totalDisplayData = totalData.map((val, idx) =>
      ensureMinimumValue(val, val)
    );
    const dropDisplayData = dropData.map((val, idx) =>
      ensureMinimumValue(val, val)
    );
  ...
  if (!summaryData || Object.keys(summaryData).length === 0) {
    return (
      <Card bordered={false} className="db-chart-area">
        ...
        <div className="chart-container">No summary data available</div>
      </Card>
    );
  }
