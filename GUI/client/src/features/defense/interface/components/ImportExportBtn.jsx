...
const ImportExportBtn = ({
  ...
  const handleExportExcel = async () => {
    ...
    const createStyledSheet = async (data, sheetName, headers) => {
      const worksheet = workbook.addWorksheet(sheetName);

      // Add headers
      const headerRow = worksheet.addRow(headers);

      // Style headers: bold, background color, centered - specific colors for IP Address and Port
      headerRow.eachCell((cell, colNumber) => {
        const header = headers[colNumber - 1];
        let headerColor = "FF4F81BD"; // Default blue
        if (header === "IPv4 Address" || header === "IPv6 Address") {
          headerColor = "FF4472C4"; // Darker blue for IP Address
        } else if (header === "Port") {
          headerColor = "FF70AD47"; // Green for Port
        }
      ...
      data.forEach((row, rowIndex) => {
        const wsRow = worksheet.addRow(Object.values(row));
      ...
      worksheet.columns = headers.map((header, index) => {
        ...
        // Check data rows for max length (simple approximation)
        data.forEach((row) => {
          const cellValue = Object.values(row)[index] || "";
          maxLength = Math.max(maxLength, cellValue.toString().length);
        });
        return { header, key: header, width: Math.max(20, maxLength + 5) };
      });
    ...
    if (ipv4.length) {
      const ipv4Headers = showPort
        ? ["IPv4 Address", "Port"]
        : ["IPv4 Address"];
      const ipv4Data = ipv4.map((ip) => ({
        "IPv4 Address": ip.Address,
        ...(showPort && { Port: ip.Port || "" }),
      }));
      await createStyledSheet(ipv4Data, "IPv4", ipv4Headers);
    }

    if (ipv6.length) {
      const ipv6Headers = showPort
        ? ["IPv6 Address", "Port"]
        : ["IPv6 Address"];
      const ipv6Data = ipv6.map((ip) => ({
        "IPv6 Address": ip.Address,
        ...(showPort && { Port: ip.Port || "" }),
      }));
      await createStyledSheet(ipv6Data, "IPv6", ipv6Headers);
    }
