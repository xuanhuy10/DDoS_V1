...
import {
  ...
  createMirroringDeviceInterface,
...
export default function MirroringAdd({ onSaved, isModalOpen, onCancel }) {
  ...
  useEffect(() => {
    ...
    if (!mirrorTypeValues || mirrorTypeValues.length === 0) {
      ...
      if (mirrorTypeValues.includes("Source Port"))
        keepFields.push("SourcePort");
      if (mirrorTypeValues.includes("Dest IP"))
        keepFields.push("DestIPv4", "DestIPv6");
      if (mirrorTypeValues.includes("Source IP"))
        keepFields.push("SourceIPv4", "SourceIPv6");
      if (mirrorTypeValues.includes("Protocol")) keepFields.push("Protocol");
      const removeFields = allDynamicFields.filter(
        (f) => !keepFields.includes(f)
      );
  ...
  const handleSave = async () => {
    try {
      ...
      const mirroringInterface = interfaces.find(
        (i) => i.InterfaceId === values.MirroringInterface
      );
  ...
  const isDeepEqual = (a, b) => {
    return (
      JSON.stringify(normalizeFields(a)) === JSON.stringify(normalizeFields(b))
    );
  };
