...
export default function Analyze() {
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const pdf = new jsPDF("portrait", "pt", "a4", true); // Compress=true
      ...
      const addHeader = async () => {
        ...
        return new Promise((resolve, reject) => {
          img.onload = () => {
            ...
            const logoBase64 = logoCanvas.toDataURL("image/jpeg", 0.2);
            ...
            pdf.setFontSize(16);
            pdf.text("Network Analytics Report", margin, y + 12);
            pdf.setFontSize(9);
      ...
      // Traffic Chart - Điều chỉnh height sát để full page, tránh tràn
      const addTrafficChart = async () => {
        const availableHeight = pageHeight - y - margin - sectionSpacing;
        addNewPageIfNeeded(availableHeight);
        ...
        if (trafficChartCanvas) {
          // Dynamic height: Sát với available space, max 70% page để full nhưng không tràn
          const canvasRect = trafficChartCanvas.getBoundingClientRect();
          let imgHeight = Math.min(
            availableHeight * 0.7, // Giới hạn 70% available để full + spacing
            (canvasRect.height / canvasRect.width) * (pageWidth - 2 * margin)
          );
          imgHeight = Math.max(imgHeight, 100); // Min height để không quá nhỏ

          const trafficCanvas = await html2canvas(trafficChartCanvas, {
            scale: 0.8, // Giảm scale từ 1.0 xuống 0.8 để size nhỏ hơn, fit tốt
            useCORS: true,
            allowTaint: true,
            backgroundColor: null, // Không set background để giữ transparent nếu cần
          });
          const imgWidth = pageWidth - 2 * margin;
          const trafficBase64 = trafficCanvas.toDataURL("image/png", 0.5);
          ...
          // Fallback text với height nhỏ để không tràn
          pdf.setFont("helvetica", "normal");
          pdf.setFontSize(9);
          pdf.text("Chart unavailable - see dashboard.", margin, y + 8);
          y += 25 + sectionSpacing;
        }
      };
      ...
      const addIPTable = (title, data) => {
        const sortData = data.sort((a, b) => b.bits - a.bits).slice(0, 3);
        ...
        autoTable(pdf, {
          ...
          didParseCell: (data) => {
            ...
            if (ipColors[ip]) data.cell.styles.fillColor = ipColors[ip];
          },
        });
        y = pdf.lastAutoTable.finalY + sectionSpacing;
      };
      ...
      const addTrendsTables = () => {
        ...
        const protocolBody = formatProtocolTrend();
        autoTable(pdf, {
        ...
        const attackBody = formatAttackTrend();
        autoTable(pdf, {
          startY: y + subSectionSpacing,
        ...
        y = Math.max(pdf.lastAutoTable.finalY || y) + sectionSpacing;
      };
      ...
      const fileName = `Network_Analytics_${fromDate}_${toDate}.pdf`;
      pdf.save(fileName);
