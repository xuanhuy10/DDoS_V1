...
import { Button, Typography, Space, message, Modal, Upload, Col } from "antd";
import {
  PlusOutlined,
  DeleteOutlined,
  InboxOutlined,
...
import {
  ...
  getAllHTTPBlockedAddresses,
  importDeleteBulkHTTPBlockedAddresses,
...
export default function VPNWhitelist() {
  ...
  useEffect(() => {
    async function fetchData() {
      const res = await getAllHTTPBlockedAddresses();
      setIpList(res.data || []);
    }
    fetchData();
  }, [refreshKey]);
  ...
  const handleCreate = async (newIp) => {
    try {
      ...
      await insertHTTPBlockedAddresses({
  ...
  const handleImportDeleteApply = async () => {
    ...
    try {
      ...
      await importDeleteBulkHTTPBlockedAddresses(toDeleteObjects);
      const res = await getAllHTTPBlockedAddresses();
      if (res && res.data) setIpList(res.data);
      setSelectedRowKeys([]);
      message.success(
  ...
  return (
    <InterfaceLayout selectedKey="http_black">
      <Title level={3}>HTTP/HTTPs Blacklist</Title>
      <Paragraph>
        Auto block IP addresses that are trying to access your network. The
        addresses inside the list are being blocked.
      </Paragraph>
      <Space>
        <Button
          icon={<PlusOutlined />}
