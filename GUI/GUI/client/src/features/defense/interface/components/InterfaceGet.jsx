...
export default function PortInterface() {
      ...
      setOriginalInterfaces(
        rawInterfaces.reduce((acc, iface) => {
          acc[iface.InterfaceId] = {
            InterfaceType: iface.InterfaceType || "inbound",
        ...
        ports: [
          {
            ...
            InterfaceType:
              updateData.InterfaceType || iface.InterfaceType || "inbound",
  ...
  const hasActiveIpSecProfile = (interfaceId) => {
    return ipSecProfiles.some(
      (profile) => profile.InterfaceId === interfaceId && profile.Enable === 1
    );
  };
    ...
    },
    {
      ...
      render: (text, record) => (
        <Space>
          ...
          {hasActiveIpSecProfile(record.InterfaceId) && (
            <Tooltip title="Protecting">
              <SafetyOutlined style={{ color: "#FF8C00", fontSize: 17 }} />
            </Tooltip>
          )}
        </Space>
      ),
    },
    {
      ...
      render: (text, record) => (
        <Space>
          ...
          {hasActiveIpSecProfile(record.InterfaceId) && (
            <Tag color="#FF8C00">Protecting</Tag>
          )}
        </Space>
      ),
    },
